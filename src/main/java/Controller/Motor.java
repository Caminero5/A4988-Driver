...
public class Motor{
    ...
    public void rotate(boolean clockwise, boolean ms1, boolean ms2, boolean ms3, String STEP, String MS1, String MS2, String MS3, String DIR, String step_delay, String limit1, String limit2, long seconds) throws IOException, InterruptedException {
        Gpio_On clockwiseOn = new Gpio_On(DIR, "DIR on");
        Gpio_Off clockwiseOff = new Gpio_Off(DIR, "DIR off");

        //checks if the value is true or false then sets the pin to that
        if(clockwise) {
            clockwiseOn = new Gpio_On(DIR, "DIR on");
            clockwiseOn.begin(DIR, "DIR on");
        }
        else {
            clockwiseOff = new Gpio_Off(DIR, "DIR off");
            clockwiseOff.begin(DIR, "DIR off");
        }
        ...
        GPIO_On_Till_Press gpioOnTillPress = new GPIO_On_Till_Press(STEP, limit1, step_delay);

        //starts motor thread
        gpioOnTillPress.begin(STEP, limit1, step_delay);

        Thread.sleep(seconds);

        if(clockwise) clockwiseOff.begin(DIR, "clockwise off");
        else {clockwiseOn.begin(DIR, "clockwise On"); }

        GPIO_On_Till_Press gpio_on_till_press2 = new GPIO_On_Till_Press(STEP, limit2, step_delay);
        gpio_on_till_press2.begin(STEP, limit2, step_delay);
    }

    public void rotate(boolean clockwise, boolean ms1, boolean ms2, boolean ms3, String STEP, String MS1, String MS2, String MS3, String DIR, String step_delay, String btn_pin, String led_pin) throws IOException {
